...
tmp <- titanic
tmp$survived <- as.integer(titanic$Survived)
...
Survived = predict(arvore, newdata = titanic_test)
Survive = Survived[,2]>.5


submission <- data.frame(PassengerId =titanic_test$PassengerId,
                         Survived = as.integer(Survive))
...
# Matriz de confusão
tab1 <- table(Survive, titanic$Survived)
tab1
